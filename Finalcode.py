...
def initialTransformations(_image):
    ...
    _image = cv2.bilateralFilter(_image, 9, 10 ,20 ) #preserve edge and remove noise
    filters = _image
    _image2=_image
    width = 650
    height = 550
    ...
    # resize image
    resized = cv2.resize(_image2, dim, interpolation = cv2.INTER_AREA)
    cv2.imshow("bilateral filter",resized)
    _image = cv2.GaussianBlur(_image,(11,11) , 4) #
    blur = _image
    _image2=_image
    resized = cv2.resize(_image2, dim, interpolation = cv2.INTER_AREA)
    cv2.imshow("GaussianBlur filter",resized)
    ...
    _thresh2=_thresh
    resized = cv2.resize(_thresh2, dim, interpolation = cv2.INTER_AREA)
    cv2.imshow("medianBlur filter",resized)
    thresh = _thresh
    _image = _thresh
...
label1 = Label(root, text='fake currency',font=("Arial Bold",14))
...
image = Image.open(cwd+"/bg.jpg")
photo = ImageTk.PhotoImage(image)
label = Label(root, image=photo)
label.image = photo  # keep a reference!
label.pack()

label2 = Label(root, text='fake currency',font=("Arial Bold",14))
...
root.geometry('500x500') # Size 200, 200
root.resizable(width=True, height=True)


Button(text='detect', command=callback).pack(side=TOP, padx=50)
